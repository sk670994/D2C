...
$Backup = Join-Path (Get-Location) ("adspy-v25-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss"))

$Targets = @(
  "components\dashboard\adspy\AdSpySection.tsx",
  "components\dashboard\adspy\adspy-refinement-v23.css",
  "components\dashboard\adspy\adspy-refinement-v25.css",
...
foreach ($target in $Targets) {
  ...
  $dst = Join-Path (Get-Location) $target
  if (-not (Test-Path $src)) { throw "Missing v25 file: $target" }
  New-Item -ItemType Directory -Force -Path (Split-Path $dst) | Out-Null
  Copy-Item $src $dst -Force
}

Write-Host "AdSpy v25 applied."
