...
$Backup = Join-Path (Get-Location) ("adspy-v21-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
foreach ($target in $Targets) {
  ...
  $dst = Join-Path (Get-Location) $target
  if (-not (Test-Path $src)) { throw "Missing v21 file: $target" }
  New-Item -ItemType Directory -Force -Path (Split-Path $dst) | Out-Null
  Copy-Item $src $dst -Force
}

Write-Host "AdSpy v21 applied."
