-- AdSpy context graph (v1). Idempotent.
-- NOTE: not written during collection yet (see lib/ad-intelligence/hawky/context.ts);
-- the tables exist so the graph can be backfilled offline later.
...
-- Server-side only. RLS on with no policies: anon/authenticated see nothing,
-- the service role bypasses RLS. (RLS disabled on a public table is flagged
-- as a security error by the Supabase advisor.)
alter table public.adspy_context_nodes enable row level security;
alter table public.adspy_context_edges enable row level security;
...
VALUES ('20260926020000', 'adspy_context_graph_v1')
